...
public:
    ...
    ~SneakersShop() {
        delete [] sneakers; //TODO be careful!
    }
    ...
    SneakersShop & operator++() {
        for (int i=0;i<n;i++){
            sneakers[i]*=(1+INCREASMENT/100.0);
//            sneakers[i].setPrice(sneakers[i].getPrice()*(1+INCREASMENT/100.0))
//            sneakers[i]+=(sneakers[i].getPrice()/100.0*INCREASMENT);
        }
        return *this;
    }
    ...
    static void setIncreasePercentage(int increasment) {
        INCREASMENT = increasment;
    }

    Sneakers & operator [](int size){
        for (int i=0;i<n;i++){
            if (sneakers[i].getSize()==size){
                return sneakers[i];
            }
        }
        //throw an exception because no sneakers fit the size
//        throw ;
    }
};
...
ostream &operator<<(ostream &os, const SneakersShop &shop) {
    os << "Sneakers shop: " << shop.name << endl << "List of sneakers:\n";
    if (shop.n==0){
        cout << "EMPTY" << endl;
    } else {
        for (int i=0;i<shop.n;i++){
            cout << i+1 << ". " << shop.sneakers[i] << endl;
        }
    }
    return os;
}
...
private:
    char url [50];
    const static int DISCOUNT = 10;
    const static double DISCOUNT_COEF;
public:
    ...
    float checkItemAvailability(char *model, int size) override {
        return SneakersShop::checkItemAvailability(model, size) * (1 - DISCOUNT/100.0);
//        return SneakersShop::checkItemAvailability(model, size) * DISCOUNT_COEF;
    }

    friend ostream &operator<<(ostream &os, const OnlineSneakersShop &shop);
};

const double OnlineSneakersShop::DISCOUNT_COEF = 0.9;

ostream &operator<<(ostream &os, const OnlineSneakersShop &shop) {
    os << "Online sneakers shop: " << shop.name << " URL: " << shop.url << endl << "List of sneakers:\n";
    if (shop.n==0){
        cout << "EMPTY" << endl;
    } else {
        for (int i=0;i<shop.n;i++){
